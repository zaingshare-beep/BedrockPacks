...
/* ================= DOWNLOADS ================= */
const files = {
    "Fantasy Add-on":"packs/fantasy-addon.mcpack",
    "RTX Pack":"packs/rtx-pack.mcpack",
    "SkyBlock":"packs/skyblock.mcpack",
    "Fantasy Pack":"packs/fantasy-addon.mcpack"
};
window.download = function(name) {
    if (!files[name]) return alert("Pack file not configured.");
    const a = document.createElement("a");
    a.href = files[name];
    a.download = files[name].split("/").pop();
...
if (feedbackForm) {
    feedbackForm.addEventListener("submit", async function (e) {
        ...
        try {
            const response = await fetch("https://formsubmit.co/ajax/zain.gshare@gmail.com", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    rating: rating,
                    message: message,
                    _subject: "New BedrockPacks Feedback",
                    _template: "table"
                })
            });
